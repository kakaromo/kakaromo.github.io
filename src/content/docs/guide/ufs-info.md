...
UFS 참조 데이터는 Test Request(TR) 생성 시 드롭다운 선택 항목으로 사용되는 코드 테이블입니다. 페이지 경로: `/ufsinfo`
...
각 코드 테이블에 대해 다음 작업을 수행할 수 있습니다:
...
:::tip
새로운 UFS 모델이나 사양이 추가되면 먼저 이 페이지에서 참조 데이터를 등록한 후 TR을 생성하세요.
:::

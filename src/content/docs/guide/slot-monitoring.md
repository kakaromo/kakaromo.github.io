---
title: 실시간 슬롯 모니터링
description: 슬롯 상태 실시간 확인, 다중 선택, Context Menu를 통한 테스트 제어 방법
---

Slots 페이지는 테스트 슬롯의 실시간 상태를 모니터링하고, 슬롯에 대한 다양한 제어 작업을 수행하는 핵심 페이지입니다.

## 슬롯 카드 UI

각 슬롯은 카드 형태로 표시되며, 현재 상태에 따라 색상이 달라집니다.

| 상태 | 색상 토큰 | 설명 |
|------|-----------|------|
| **pass** | emerald | 테스트 통과 |
| **warning_pass** | amber | 경고 포함 통과 |
| **warning** | amber | 경고 |
| **fail** | red | 테스트 실패 |
| **running** | emerald | 테스트 실행 중 (애니메이션) |
| **booting** | cyan | 부팅 중 (애니메이션) |
| **stop** | gray | 정지 |
| **disconnect** | slate | 연결 끊김 |

:::note
상태별 색상은 `$lib/config/slotState.ts`에서 중앙 관리됩니다. 정확한 문자열 매칭 외에 **contains fallback**도 지원하여, `timeout_fail`, `booting_fail`, `warning_pass` 등 복합 상태도 자동으로 적절한 색상이 적용됩니다.
:::

슬롯 카드에는 슬롯 번호, 현재 상태, 할당된 TR/TC 정보, 그리고 **product** 정보가 표시됩니다. product는 `Controller_NandType_CellType_NandSize_Density` 형식으로 조합된 계산 필드입니다.

### 슬롯 카드 툴팁

슬롯 카드에 마우스를 올리면 툴팁이 표시되어 해당 슬롯의 전체 상세 정보를 확인할 수 있습니다. 툴팁에는 modelName, battery, testState, setLocation, trName, runningState 등 HeadSlotData의 모든 주요 필드가 포함됩니다.

## 탭

페이지 상단에 Head 연결 목록 기반으로 탭이 동적 생성됩니다. 각 탭은 특정 Head에 연결된 슬롯 그룹을 표시합니다. 탭을 클릭하여 원하는 Head의 슬롯을 확인합니다.

## 슬롯 선택

다양한 방법으로 슬롯을 선택할 수 있습니다:

| 방법 | 동작 |
|------|------|
| **클릭** | 단일 슬롯 선택 (기존 선택 해제) |
| **Ctrl + 클릭** | 기존 선택 유지하며 추가/해제 |
| **Shift + 클릭** | 마지막 선택부터 현재까지 범위 선택 |
| **드래그** | 드래그 영역 내 슬롯 범위 선택 |
| **Ctrl + A** | 현재 탭의 모든 슬롯 선택 |

## Context Menu

슬롯을 우클릭하면 Context Menu가 표시됩니다.

| 메뉴 항목 | 설명 |
|-----------|------|
| **Terminal** | 해당 슬롯의 VM으로 원격 터미널 열기 |
| **Log Browser** | 슬롯 로그 디렉토리 브라우저 열기 |
| **Test Start** | 선택된 슬롯에서 테스트 시작 |
| **Test Stop** | 실행 중인 테스트 중지 |
| **Set TR** | TR 할당 |
| **Set TC** | TC 할당 |
| **MakeSet** | FW/Provision/Image 배치 설정 — 다중 슬롯 시 [MakeSet 그룹](/guide/makeset-groups/) 기반 자동 매칭 |
| **Debug > {type}** | DB `debug_types`에서 동적 생성. 현재 [DLM](/guide/dlm-debug/) 지원 (USB ID 필요) |
| **Clear** | 슬롯의 TR/TC 할당 해제 |
| **Edit UFS** | UFS 정보 편집 |

:::caution
Test Stop은 실행 중인 테스트를 강제로 중지합니다. 진행 중인 데이터가 손실될 수 있으므로 주의하세요.
:::

## Selection Sheet

슬롯을 하나 이상 선택하면 화면 하단에 **Selection Sheet** 액션 패널이 나타납니다. Context Menu와 동일한 작업을 버튼으로 제공하며, 다중 선택된 슬롯에 일괄 작업을 수행할 수 있습니다. 각 슬롯 항목 옆에 Log Browser 버튼(폴더 아이콘)이 있어 해당 슬롯의 로그 디렉토리를 바로 열 수 있습니다.

## TC Group 빠른 적용

Set TC 작업 시 SetTC Sheet에서 저장된 TC 그룹을 Chip으로 빠르게 적용할 수 있습니다. 자세한 내용은 [TC 그룹](/guide/tc-groups/) 페이지를 참고하세요.

## MakeSet 그룹 — 다중 보드 배치 MakeSet

여러 종류의 보드가 포함된 슬롯을 선택하여 MakeSet을 수행할 때, MakeSet 그룹을 사용하면 보드별 Provision/Image 설정이 자동으로 매칭됩니다. 자세한 내용은 [MakeSet 그룹](/guide/makeset-groups/) 페이지를 참고하세요.

## RUNNING TC 실시간 성능 데이터

실행 중(RUNNING)인 슬롯 카드를 확장하면 현재 진행 중인 테스트의 **실시간 성능 데이터**를 확인할 수 있습니다. 테스트 완료를 기다리지 않고 진행 상황을 즉시 파악할 수 있습니다.

:::tip
슬롯 상태는 SSE(Server-Sent Events)를 통해 실시간으로 업데이트됩니다. 별도의 새로고침이 필요 없습니다.
:::

## 성능 TC 비교 (VS 버튼)

슬롯 페이지에서 완료된 성능 TC의 결과를 바로 비교할 수 있습니다. 별도의 성능 비교 페이지로 이동하지 않고 슬롯 컨텍스트 안에서 비교 워크플로를 완결합니다.

### CompareSheet 열기

성능 탭의 할당된 TC 테이블에는 **VS** 컬럼이 표시됩니다. 결과가 있는 행(historyId가 존재하는 행)에서 VS 버튼을 클릭하면 `CompareSheet`가 직접 열리며, 해당 TC의 성능 결과가 비교 기준(Baseline)으로 자동 설정됩니다.

:::note
VS 컬럼은 성능 탭에서만 표시됩니다. 호환성 탭에서는 비교 기능이 비활성화됩니다.
:::

### 다른 FW 추가 비교

CompareSheet가 열리면 내장된 FW Picker를 통해 비교할 다른 펌웨어 버전의 결과를 추가할 수 있습니다. 추가된 ID 목록은 `onIdsChange` 콜백으로 관리되며, 여러 FW 버전을 동시에 오버레이하여 성능 변화를 시각적으로 확인할 수 있습니다.

### 슬롯 확장 행에서 Reparse

완료된 TC의 슬롯 확장 행(expanded row)에는 **Reparse** 버튼이 표시됩니다. 이 버튼을 클릭하면 해당 TC의 로그 파일을 다시 파싱하여 성능 데이터를 재생성합니다.

- **Reparse 진행 중**: 버튼이 비활성화되고 회전 아이콘과 함께 "Reparsing..." 텍스트가 표시됩니다.
- **진행 상태**: 글로벌 `reparseStore`를 통해 추적되며, 화면 우하단의 Floating Card에서 전체 진행 상황을 확인할 수 있습니다.
- **조건**: 로그 경로(`logPath`)가 존재하고, 현재 테스트가 실행 중(RUNNING)이 아닌 TC에서만 Reparse 버튼이 표시됩니다.

:::tip
Reparse는 파싱 로직이 업데이트되었거나, 기존 파싱 결과에 오류가 있을 때 사용합니다. 원본 로그 파일은 변경되지 않으며, 파싱 결과만 새로 생성됩니다.
:::

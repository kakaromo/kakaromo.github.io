...
| **FW Code Linux Base** | Linux 측 FW 코드 기본 경로 | `/mnt/nas/FW_Code` |
| **FW Code Windows Base** | Windows 측 FW 코드 기본 경로 | `F:\FW_Code` |
| **Result Base Path** | Linux 측 결과 저장 기본 경로 | `/mnt/nas/T32_Results` |
| **Result Windows Base Path** | Windows 측 결과 저장 기본 경로 | `F:\T32_Results` |
...
| `{slot}` | `1` | 슬롯 번호 |
| `{result_path}` | `F:\T32_Results\T10-1_FW_20260407` | 결과 저장 경로 (Windows) |
| `{branch_path}` | `F:\FW_Code\SERRA\branch_name` | FW 코드 경로 (Windows) |

### 경로 매핑 (FW Code Path Mapping)

T32 Dump 시 Linux 경로를 Windows 경로로 자동 변환합니다:

```
Linux:   /mnt/nas/FW_Code/SERRA/branch_name
         ↓ fwCodeLinuxBase → fwCodeWindowsBase 치환
Windows: F:\FW_Code\SERRA\branch_name
         ↓ "/" → "\" 경로 구분자 변환
Result:  "F:\FW_Code\SERRA\branch_name"
```

- Dump Dialog에서 Bitbucket에서 다운로드한 브랜치 폴더를 선택할 수 있습니다
- 선택한 폴더 경로가 `{branch_path}` 플레이스홀더로 dump 명령어에 전달됩니다
- 결과 경로는 `{result_path}` 플레이스홀더로 전달됩니다 (`{resultWindowsBasePath}/{tentacle}-{slot}_{fw}_{date}/` 형식)
...
| 4 | **완료** | 결과 경로 표시 + 결과 파일 미리보기 + Download 버튼 |

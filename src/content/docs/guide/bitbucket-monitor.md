...
3. **자동 다운로드** 모드에 따라:
   - `autoDownload=true` (기본): 즉시 ZIP 다운로드 + 압축 해제
   - `autoDownload=false`: **DETECTED** 상태로만 기록 (용량 관리용)
4. 브랜치명에서 **UFS Controller를 자동 감지** (DB 매칭)
5. 해당 브랜치의 소스를 **ZIP으로 다운로드** → 저장 경로에 저장 → **압축 해제 후 ZIP 삭제** (폴더만 보존)
6. 브랜치의 **커밋 날짜**(commitDate)를 Bitbucket API에서 가져와 DB에 기록
...
| **Auto Download** | 새 브랜치 감지 시 자동 다운로드 여부 | 체크 (기본: ON) |
| **Enabled** | 자동 폴링 활성화 여부 | 체크 |

### Auto Download 설정

- **ON (기본)**: 새 브랜치 감지 시 자동으로 ZIP 다운로드 + 압축 해제
- **OFF**: 새 브랜치를 **DETECTED** 상태로만 기록 (디스크 용량 관리 시 유용)
  - DETECTED 상태의 브랜치는 이력 패널에서 개별적으로 다운로드 가능
...
- **새 브랜치 감지 (autoDownload=true)**: ZIP 다운로드 → 압축 해제 → DB에 이력 기록
- **새 브랜치 감지 (autoDownload=false)**: DETECTED 상태로 DB에만 기록 (다운로드 없음)
...
### DETECTED 브랜치 개별 다운로드

`autoDownload=false`로 설정된 저장소의 DETECTED 브랜치를 개별적으로 다운로드할 수 있습니다.

1. 브랜치 이력 패널에서 DETECTED 상태의 브랜치 행 클릭
2. **다운로드 아이콘** 클릭
3. **플로팅 카드**가 우하단에 나타나며 실시간 진행률 표시 (SSE 스트리밍)
4. 다운로드 완료 후 상태가 `DOWNLOADED`로 변경

:::note
플로팅 카드는 여러 브랜치를 동시에 다운로드할 때도 각각의 진행 상황을 표시합니다.
:::

### 브랜치 파일 삭제

다운로드된 브랜치의 파일을 삭제하여 디스크 공간을 확보할 수 있습니다.

1. 브랜치 이력 패널에서 DOWNLOADED 상태의 브랜치 행 클릭
2. **삭제 아이콘** 클릭
3. ZIP 파일과 압축 해제된 폴더가 모두 삭제됨
4. 상태가 `DETECTED`로 변경되며, 필요 시 재다운로드 가능

### 브랜치 이력 삭제

브랜치 DB 기록 자체를 삭제합니다.

1. 브랜치 이력 패널에서 브랜치 행 선택
2. **휴지통 아이콘** 클릭
3. DB 기록이 완전히 삭제됨
...
| 커밋 날짜 조회 | `GET /rest/api/latest/projects/{proj}/repos/{repo}/commits?until={branchId}&limit=1` |

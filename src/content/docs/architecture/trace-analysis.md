...
flowchart TD
    subgraph Browser["Browser (SvelteKit + Svelte 5)"]
        UI["/trace 페이지\n3-pane: Jobs | Chart/Stats"]
        ECharts["ECharts scatter 렌더러\n(기본)"]
        Deck["Deck.gl WebGL 렌더러\n(feature flag, 1M+ 포인트)"]
        Arrow["apache-arrow\n(Arrow IPC 디코더)"]
    end

    subgraph Portal["Portal (Spring Boot)"]
        Ctrl["TraceController\nTraceJobController"]
        Svc["TraceJobService\n@Async 파싱 orchestration"]
        DB[("binmapper DB\nportal_trace_jobs\nportal_trace_parquets")]
    end

    subgraph Rust["Rust trace 서비스 (:50053)"]
        GRPC["gRPC server\nProcessLogs\nGetChartData\nGetTraceStats"]
        Parser["log parser\n(ftrace / blktrace CSV / UFSCUSTOM)"]
        Proc["processors\nlatency/continuous/aligned 계산"]
        PqSync["parquet sync reader\n(/tmp download)"]
        PqAsync["parquet async reader\n(MinIO range-GET)"]
    end

    subgraph MinIO["MinIO (:9000)"]
        Uploads["trace-uploads/\n원본 로그"]
        Parquet["trace-parquet/\n생성된 parquet\n{userId}/{jobId}/{ufs,block,ufscustom}.parquet"]
    end

    UI -->|"REST /api/trace/*"| Ctrl
    Ctrl --> Svc
    Svc -->|JPA| DB
    Svc -->|"gRPC ProcessLogs (streaming progress)"| GRPC
    Svc -->|"gRPC GetChartData (unary, Arrow IPC)"| GRPC
    Svc -->|"gRPC GetTraceStats (unary, JSON)"| GRPC

    Svc -->|"MinIO PUT (원본)"| Uploads
    GRPC -->|"MinIO GET (원본)"| Uploads
    GRPC -->|"MinIO PUT (parquet)"| Parquet
    PqSync -->|"전체 다운로드 → /tmp"| Parquet
    PqAsync -->|"footer + row group range-GET"| Parquet
    Parser --> Proc --> PqSync
    Proc --> PqAsync

    UI --> ECharts
    UI --> Deck
    Ctrl -->|"Arrow IPC binary"| Arrow
    Arrow --> ECharts
    Arrow --> Deck
```
...
| Method | Path | 용도 | 응답 |
|---|---|---|---|
| `POST` | `/api/trace/upload` | 로그 파일 업로드 + 파싱 트리거 | `{ success, jobId, status }` |
...
| `POST` | `/api/trace/jobs/{id}/reparse` | 기존 원본으로 재파싱 (파서 로직 갱신 시) | `{ success, jobId, status }` |
...
| `continuous` | Bool | 이전 complete 의 `lba+size == lba` |
| `aligned` | Bool | `lba%8==0 && size%8==0` (UFS 4KB 정렬) |

#### Block (block-v1)

- UFS 와 구조 동일, 단 `lba` → `sector` 로 naming. `opcode` → `io_type` (RA/R/W/FF/D 등). `chart_rpc::build_chart_batch_block` 에서 `sector→lba`, `io_type→cmd` 로 **공용 스키마로 리네임**.
- Flush/size=0/sector=u64::MAX 케이스는 **파싱 단계에서 `sector=0` 정규화** (커밋 `4046671`, `log_common::normalize_sector`).
...
sequenceDiagram
    ...
    participant P as TraceJobController
    participant S as TraceJobService
    participant DB as binmapper DB
    participant MIO as MinIO
    participant R as Rust trace (:50053)

    B->>P: POST /api/trace/upload (multipart)
    P->>S: uploadAndParse(user, file)
    S->>DB: INSERT (status=UPLOADED, progress=0)
    DB-->>S: jobId
    S->>MIO: PUT trace-uploads/{user}/{date}/{jobId}/{file}
    S->>P: TraceJob
    P-->>B: { jobId, status=UPLOADED }

    Note over S: @Async triggerParse(jobId)
    S->>DB: status=PARSING
    S->>R: ProcessLogs (streaming gRPC)
    R->>MIO: GET trace-uploads/...
    R->>R: parse + processors(latency/qd/continuous)
    R->>MIO: PUT trace-parquet/{user}/{jobId}/{ufs,block,ufscustom}.parquet
    R-->>S: ProcessLogsProgress (stage/progress/…)
    S->>DB: currentStage, progressPercent 갱신
    R-->>S: 완료 (output_files[])
    S->>DB: INSERT portal_trace_parquets rows
    S->>DB: status=PARSED, parsedAt=now

    B->>P: GET /api/trace/jobs/{id} (polling 2s)
    P-->>B: status=PARSED + parquets
```

### 주요 포인트

- **선 INSERT 후 업로드**: jobId 를 먼저 발급받아 MinIO 경로에 포함 → id 기반 고유성 보장
- **@Async 파싱**: upload 요청은 즉시 응답 반환, 파싱은 백그라운드 스레드 (Portal 재시작 시 PARSING 중이던 job 은 그대로 FAILED 로 가지 않고 다음 폴링 시점까지 마지막 상태 유지 — 복구는 `POST /reparse` 로 수동)
...
```mermaid
flowchart LR
    A[요청: parquetId<br/>+ filter<br/>+ timeRange<br/>+ targetPoints] --> B[Portal<br/>TraceController]
    B -->|"gRPC GetChartData"| C[Rust<br/>chart_rpc]
    C --> D["parquet projection<br/>+ row-filter (time)"]
    D --> E["apply_filters<br/>(action/lba/qd/dtoc/ctoc/ctod)"]
    E --> F[build_chart_batch_<br/>ufs/block/ufscustom]
    F --> G[time_bucket_decimate]
    G --> H[compute_stats]
    F --> H
    G --> I[Arrow IPC<br/>StreamWriter]
    H --> J[ChartLatencyStats<br/>proto]
    I --> K[bytes arrow_ipc]
    J --> L[headers<br/>X-Trace-Stats JSON]
    K --> M[Portal 응답]
    L --> M
    M -->|"application/vnd.apache.arrow.stream"| N[Browser<br/>tableFromIPC]
```
...
```mermaid
flowchart TD
    A[parquet] --> B[apply_filters]
    B --> C[chart batch<br/>공용 10컬럼]
    C --> D1[total / send_count / duration]
    C --> D2[continuous / aligned ratio]
    C --> D3[read/write/discard total bytes]
    C --> D4[Latency 4종<br/>dtoc/ctod/ctoc/qd<br/>min/max/avg/stddev/median<br/>p99/p999/p9999/p99999/p999999]
    C --> D5[CMD 별 stats<br/>count/send/ratio/totalSize<br/>+ 4 latency]
    C --> D6[Latency Histogram<br/>cmd × lt 별 버킷 카운트]
    C --> D7[CMD + Size count]
    D1 & D2 & D3 & D4 & D5 & D6 & D7 --> E[JSON 응답]
```

### 6.2 핵심 구현

`output/stats_rpc.rs::build_payload` — 단일 `RecordBatch` 스캔으로 모든 집계 동시 수행. 정렬 기반 percentile (sort + linear interpolation). 별도 라이브러리 없음.

**action_pair** — trace_type / 실제 리터럴 스캔으로 request/complete action 문자열 결정:
...
**continuous 정의**: 이전 complete 이벤트의 `lba + size == 현재 이벤트 lba` (cmd 그룹 내). Parser/processor 단계에서 이미 계산된 `continuous` 컬럼이 있으면 그대로 사용, 없으면 stats 계산 시 fallback 재계산.

**sector_bytes**: `ufs`/`ufscustom`=4096, `block`=512 (read/write/discard total 바이트 환산).

### 6.3 Latency Histogram
...
File::open → ParquetRecordBatchReaderBuilder (sync) → concat_batches
```
...
```mermaid
flowchart LR
    A[Portal gRPC 요청] --> B{TRACE_PARQUET_READER}
    B -->|"unset (기본)"| C[sync 경로]
    B -->|"async"| D[async 경로]

    C --> C1[download_file<br/>전체 /tmp]
    C1 --> C2[sync<br/>ParquetRecordBatchReaderBuilder]
    C2 --> C3[concat → ChartPayload]

    D --> D1[head_object<br/>파일 크기 조회]
    D1 --> D2[tail 64KB range-GET]
    D2 --> D3[footer length 파싱]
    D3 --> D4[necessary? 추가 range-GET]
    D4 --> D5[ParquetMetaData]
    D5 --> D6["ParquetRecordBatchStream<br/>(row group 단위 async)"]
    D6 --> D7[projection + row-filter<br/>→ chunk 별 range-GET]
    D7 --> D8[stream.next().await → batch]
    D8 --> D9[concat → ChartPayload]
```
...
| `output/chart_rpc_async.rs` | UFS/Block/UFSCUSTOM chart payload async builder. `ParquetRecordBatchStreamBuilder` + projection + time row-filter |
| `output/stats_rpc_async.rs` | Stats async builder. `continuous` 컬럼 optional 처리 (구 parquet 호환) |
| `grpc/server.rs` | `get_chart_data`/`get_trace_stats` 핸들러 양쪽에 sync/async 분기. env `TRACE_PARQUET_READER=async` 옵트인 |

### 7.4 Footer 2-step fetch

Parquet 파일 구조:
```
[magic 'PAR1'][data row groups...][metadata][4-byte footer length][magic 'PAR1']
```

`MinioParquetReader::get_metadata`:

1. Tail 64KB 를 먼저 range-GET
2. 마지막 8바이트에서 `u32 footer_length` 파싱
3. `footer_length + 8 ≤ 64KB` 이면 tail 에서 추출
4. 넘으면 추가 range-GET 으로 앞부분 보강

→ S3 요청 **1~2회**로 metadata 완성. 전체 파일 다운로드 X.

### 7.5 검증 결과

- `/tmp/chart_data_*.parquet` 생성 0건 확인
...
```typescript
interface Props {
	series: { time, lba, qd, cpu, dtoc, ctoc, ctod, action, cmd: string[] };
	meta: ChartMeta | null;
...
- cmd 색상: `cmdColors.ts::createCmdColorAssigner` — SCSI opcode + 문자열 매칭 → Read(파랑)/Write(주황)/Flush(초록)/Discard(보라)/Other(회색) 팔레트
- Legend (우측 세로) 토글 → 모든 차트 동기화 (`setOption({legend:{selected}})` 브로드캐스트)
- Brush 드래그 (우클릭 → 영역선택 메뉴 → pointer drag → pixel→data 역변환) → `onBrushSelected` → 상위에서 filter 반영
...
X 범위는 모든 차트 동일 — `timeStartMs` / `timeEndMs` 에 저장. Filter bar 자동 펼침 + Chart + Stats 자동 재조회.

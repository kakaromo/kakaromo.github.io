...
| 클래스 | 역할 |
|--------|------|
| `HeadConnection` | DB 엔티티 (portal_head_connections) |
| `HeadConnectionRepository` | JPA Repository |
| `HeadConnectionService` | 연결 정보 CRUD |
| `HeadConnectionProperties` | YAML 설정 바인딩 (reconnect, SSE) |
| `HeadConnectionManager` | 모든 HeadTcpClient 생명주기 관리 (DB 기반) |
| `HeadTcpClient` | TCP 연결 및 명령 전송 |
| `HeadMessageParser` | 수신 메시지 파싱 |
| `HeadSlotData` | 슬롯 상태 DTO |
| `HeadSlotStateStore` | 인메모리 상태 저장소 |
| `HeadSseController` | SSE 스트리밍 API |
| `HeadCommandController` | 명령 전송 API |

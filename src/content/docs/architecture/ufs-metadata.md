...
```
┌─────────────┐    TCP     ┌──────────────┐    ADB/USB    ┌──────────┐
│    HEAD      │ ────────► │  Tentacle VM │ ────────────► │ UFS      │
│   Server     │           │  (SSH/ADB)   │  adb shell    │ Device   │
└──────┬───────┘           └──────┬───────┘               └──────────┘
       │                          │
       ▼                          ▼
┌──────────────┐           ┌──────────────┐
│ HeadSlotState│           │  JSON Files  │
│   Store      │           │ /home/octo/  │
│ (in-memory)  │           │ tentacle/    │
└──────┬───────┘           │ slot{N}/log/ │
       │                   └──────────────┘
...
```
┌───────────────────────┐     ┌──────────────────────────┐
│  ufs_metadata_types   │     │   debug_tools            │
├───────────────────────┤     │   (기존 테이블)            │
│ id (PK)               │     ├──────────────────────────┤
│ name                  │     │ id (PK)                  │
│ type_key (UNIQUE)     │     │ type_id (FK→debug_types) │
│ category              │     │ tool_name                │
│ enabled               │     │ tool_path                │
│ description           │     └────────┬─────────────────┘
...
│   └── checkSlotStateChanges() [5초 간격]              │
│       ├── startCollection() → executor에 태스크 등록  │
│       └── stopCollection() → future 취소 + 최종 수집  │
└─────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────┐
│ ScheduledExecutorService (8 daemon threads)           │
│   "metadata-collector-*"                              │
│   ├── Slot A: collectOnce() [N분 간격]                │
│   ├── Slot B: collectOnce() [N분 간격]                │
│   └── ...                                             │
└─────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────┐
│ ExecutorService (cached, daemon threads)              │
│   "metadata-cmd-timeout-*"                            │
│   └── SSH execCommand + timeout 감시                  │
│       └── Future.get(60s) → TimeoutException 시 cancel│
└─────────────────────────────────────────────────────┘
```

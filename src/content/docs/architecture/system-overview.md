---
title: 시스템 개요
description: MOVE의 프로젝트 목적, 핵심 기능, 배포 모델, 전체 구성도 및 설계 결정 근거를 설명합니다.
---

## 프로젝트 목적

MOVE은 **UFS(Universal Flash Storage) 테스트 자동화 관리 시스템**입니다. 호환성/성능 테스트의 전체 생명주기를 관리하고, 실시간 슬롯 모니터링, 원격 접속, 로그 분석, 바이너리 디버깅 도구까지 통합 제공합니다.
...
| **Metadata 모니터링** | UFS 상태 자동 수집 | TC running 중 7가지 command_type 분기 파싱 + f2fs 전용 3 뷰 |
| **Agent (디바이스 평가)** | Android 벤치마크/시나리오/trace/매크로 | Go Agent gRPC + @xyflow 시나리오 캔버스 + iotest syscall DSL |
| **Trace Analysis** | I/O trace 대용량 분석 | Rust gRPC + parquet + Arrow IPC + WebGL(Deck.gl) 1M 포인트 60fps |
...
flowchart TD
    ...
    Backend --> Excel["Go Excel Service\n(gRPC :50052)"]
    Backend --> Agent["Go Agent Service\n(gRPC :50051)"]
    Backend --> Trace["Rust Trace Service\n(gRPC :50053)"]
...
| 경로 | 프로토콜 | 용도 |
|------|----------|------|
| Browser ↔ Backend | HTTP REST + SSE + WebSocket | API, 실시간 슬롯, 원격 터미널, Agent 모니터링 |
| Browser ↔ Backend (Trace) | **HTTP + Arrow IPC bytes** | 차트 데이터를 컬럼 지향 바이너리로 전송 (Content-Type `application/vnd.apache.arrow.stream`) |
| Backend ↔ Head Server | TCP (커스텀 프로토콜) | 하드웨어 테스트 제어 |
| Backend ↔ **Go Agent** | **gRPC (protobuf) :50051** | Android ADB, 벤치마크, I/O trace, 시나리오, 매크로 |
| Backend ↔ **Go Excel Service** | **gRPC (protobuf) :50052** | Excel .xlsx 네이티브 차트 생성 |
| Backend ↔ **Rust Trace Service** | **gRPC (protobuf) :50053** | parquet 파싱, Arrow IPC 차트, 통계, MinIO range-GET |
| Backend ↔ guacd | Guacamole 프로토콜 (TCP) | SSH/RDP 중계 |
| Backend ↔ MinIO | S3 API (HTTP) | 오브젝트 스토리지 (+ Trace 서비스가 직접 range-GET) |
| Backend ↔ MySQL | JDBC | 데이터 영속화 |
| Backend ↔ Redis | Redis 프로토콜 | 캐시 |
| Backend ↔ Tentacle (SSH) | SSH (JSch) | 로그 파일 원격 접근, Metadata 수집, Pre-Command |
| Backend ↔ **ADFS** | OIDC Hybrid Flow (HTTPS) | Samsung 사내 SSO |
...
### Rust Trace Service (gRPC + Arrow IPC)

**문제**: 수 GB 규모의 I/O trace 로그를 JSON 으로 주고받으려니 페이로드·파싱 비용이 모두 폭발. Java 프로세스 안에서 parquet + Arrow 를 처리하면 **파싱이 CPU 바운드** (ftrace/blktrace 수억 라인 정규식 + 직렬 처리) 라 JVM 이 GC 로 튐.

**대안 검토**:
- Java + Apache Arrow 직접 파싱 → CPU 바운드 구간에서 JVM GC 튀는 문제 + heap 압박이 다른 API 응답성에 악영향
- 프론트엔드 파싱 → 수 GB 다운로드 자체가 비현실
- 서버가 JSON 반환 → 1M 포인트 scatter 시 20~30MB JSON → 브라우저 JSON.parse 수백 ms, 다운캐스트·TypedArray 변환 비용

**선택한 방식**: Rust 전담 서비스 + **Arrow IPC** 응답 — Samsung 사내 Go 서비스 전략(Excel/Agent)의 Rust 확장. `rayon` 병렬화 + `memmap2` 로 GB 로그를 수십 초에 parquet 변환. 차트 응답은 **컬럼 지향 TypedArrays** (`Float64Array`/`UInt32Array`) 로 JSON 대비 페이로드 50~70% 감소, 디코딩 수십 ms, WebGL 0-copy 전달 가능.

**MinIO range-GET 스트리밍 리더**: 5GB+ parquet 을 `/tmp` 경유 없이 footer 2-step fetch + row group 단위 async read. `AsyncFileReader` trait 구현으로 달성. `TRACE_PARQUET_READER=async` env 로 선택 활성화.

**트레이드오프**: 외부 서비스 3개(Agent/Excel/Trace) 로 증가. proto 동기화·배포 복잡도. 내부망에서 `.usePlaintext()` 로 단순화.

자세한 내용: [Trace Analysis 아키텍처](/architecture/trace-analysis/), [L2 20호 Trace](/learn/l2-trace/).

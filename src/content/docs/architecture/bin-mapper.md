...
```mermaid
flowchart TD
    subgraph Frontend ["Frontend (Svelte 5) — /devtools/bin-mapper"]
        INPUT["BinaryInput + StructInput + ControlBar"]
        INPUT -->|"POST /api/binmapper/parse"| VIEWER["Result Viewer:\nTableView | HexStructView | JsonView"]
    end

    subgraph Backend ["Backend (Spring Boot)"]
        CTRL["BinMapperController"] --> SVC["BinMapperService"]
        SVC --> PARSER["StructParserService"]
        SVC --> READER["BinaryReaderService"]
        PARSER --> LEXER["CppStructLexer"]
        PARSER --> CPARSER["CppStructParser"]
    end

    Frontend -->|"POST /api/binmapper/parse"| Backend
```

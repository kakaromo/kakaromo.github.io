...
### `/testdb/slot-infomations` -- 슬롯 정보 CRUD

SlotInfomation 엔티티 관리. 그룹핑 옵션 지원.

### `/testdb/sets` -- Set 정보 관리

SetInfomation (디바이스 Set) 데이터 관리.

### `/ufsinfo` -- UFS 참조 데이터

7개 코드 테이블 (CellType, Controller, Density, NandSize, NandType, OEM, UfsVersion) 관리.
...
### `/devtools/perf-generator` -- 성능 데이터 생성기

성능 차트 컴포넌트 코드를 자동 생성하는 개발 도구.

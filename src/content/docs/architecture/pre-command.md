...
|------|------|------|
| id | BIGINT PK AUTO | 고유 ID |
| source | VARCHAR(50) NOT NULL | HeadConnection name (compatibility, performance) |
| slot_index | INT NOT NULL | HEAD 내 슬롯 인덱스 (0-based) |
...
portal_slot_pre_commands (compatibility, 0):
...
HeadSlotData                       SlotInfomation
  source = "compatibility"           tentacleName = "T3"     ← setLocation "T3-0"에서 추출
  slotIndex = 0                      slotNumber = 0          ← setLocation "T3-0"에서 추출

portal_slot_pre_commands
  source = "compatibility"   ← HeadSlotData.source
  slot_index = 0             ← HeadSlotData.slotIndex
```
...
await syncTcPreCommandIds(source, slotIndex, newIds);
```

...
│   │   ├── config/                   # 글로벌 설정
│   │   ├── admin/                    # 관리자 (서버, 캐시, 헬스체크)
│   │   ├── agent/                    # Agent 벤치마크/시나리오/trace
│   │   ├── auth/                     # 인증/사용자 관리
│   │   ├── binmapper/                # BinMapper 바이너리 구조체
│   │   ├── debug/                    # 디버그 도구 (DLM 등)
│   │   ├── guacamole/                # Guacamole 원격 접속 (RDP/VNC)
│   │   ├── head/                     # Head TCP 클라이언트 + SSE
│   │   ├── logbrowser/               # 로그 브라우저 (SSH/Local)
│   │   ├── makesetgroup/             # MakeSet 그룹 관리
│   │   ├── metadata/                 # UFS 메타데이터 수집
│   │   ├── minio/                    # MinIO 스토리지
│   │   ├── tcgroup/                  # TC 그룹 관리
│   │   ├── terminal/                 # xterm.js SSH 터미널
│   │   ├── testdb/                   # TestDB 도메인 (호환성/성능)
...
│   │   └── static/                   # 빌드된 프론트엔드
│   └── proto/
│       ├── device_agent.proto        # Agent gRPC 서비스 정의
│       └── excel_service.proto       # Excel gRPC 서비스 정의
├── sql/                              # DB 스키마 SQL
├── pom.xml
└── CLAUDE.md
```

...
[adb trace_pipe] → trace.log (계속 append)
                ...
                [tail -f 폴링 50ms]
                        ↓
                [라인별 BottomHalfState 증분 처리]
                        ↓
                [윈도우(기본 1초) 경과 시 flush]
                        ↓
realtime_ufs_000001.parquet  (완료, atomic rename)
realtime_ufs_000002.parquet
realtime_block_000001.parquet
...
                        ↓
            [stop 시 타입별로 머지]
                        ↓
            ufs.parquet, block.parquet, ufscustom.parquet
```

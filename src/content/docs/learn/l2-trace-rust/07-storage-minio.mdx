...
가장 흥미로운 부분. parquet footer 구조:

```text
파일 끝 →                                   끝에서 8 바이트
                                            ┌──────────┬──────┐
[ ... row groups (대부분) ... ][ metadata ][  length   │ PAR1 │
                               ↑           │  u32_le   │ magic│
                               footer 시작 │  (4 byte) │ (4)  │
                                            └──────────┴──────┘
```

`length`를 읽기 전엔 metadata가 어디부터 시작하는지 모릅니다 (가변 길이). 그래서 두 단계:

```text
[Step 1] tail 64KB range-GET
                                           ┌────── tail 64 KB ──────┐
                                           ▼                        ▼
[ ... row groups (5GB) ............ ][ metadata ][ len ][ PAR1 ]
                                           │                  │
                                           └─ 보통 여기 모두 들어옴 ┘
                                           ↓
                                      len = u32_le(tail[-8..-4])

[Step 2A] footer가 64KB 안 → 그대로 슬라이스 (RTT 1번)
                                           ┌── slice ──┐
                                           [ metadata ][ len ][ PAR1 ]
                                           ↑ 파싱 OK

[Step 2B] footer > 64KB (큰 schema) → 한 번 더 range-GET (RTT 2번)
                                  ┌─ 2nd GET ─┐    ┌── tail (이미 받음) ──┐
                                  [ metadata ...]  [ ... metadata ][ len ][ PAR1 ]
                                                ↑ 합쳐서 파싱 OK
```

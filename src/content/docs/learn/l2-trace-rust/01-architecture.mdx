...
| `output/` | parquet/CSV/차트/통계/xlsx 직렬화 + 캐시 | `parquet.rs`(write), `reader.rs`(read + iter_*_from_parquet), `chart_rpc.rs`, `stats_rpc.rs`, `raw_rpc_duckdb.rs`(ReadParquet DuckDB), `xlsx.rs`(분할 xlsx 스트리밍), `cache.rs`(L4/L5) |

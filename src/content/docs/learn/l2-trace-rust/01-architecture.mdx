...
| `output/` | parquet/CSV/차트/통계/xlsx 직렬화 + 캐시 | `parquet.rs`(write — `(action, opcode, time)` 정렬 저장), `reader.rs`(read + iter_*_from_parquet), `chart_rpc.rs`, `stats_rpc.rs`, `raw_rpc_duckdb.rs`(ReadParquet DuckDB), `xlsx.rs`(분할 xlsx 스트리밍), `xlsx_sort_duckdb.rs`(ExportXlsx 글로벌 time 머지·정렬), `cache.rs`(L4/L5) |
| `storage/` | MinIO/S3 client | `minio_client.rs` (rust-s3 래퍼) |
| `grpc/` | gRPC 서버/클라이언트 | `server.rs`(11개 RPC), `dispatch.rs`(reader 3-way + raw page fallback), `service_helpers.rs`(stateless), `client.rs` |
| `commands/` | high-level 워크플로 (MinIO) | `handle_minio_log_to_parquet`, `handle_minio_parquet_analysis`, `handle_minio_parquet_to_csv` |
| `migration/` | 구 parquet 스키마 마이그레이션 | `run_migration` (`parquet_migrator.rs`) |
...
flowchart LR
    ...
    subgraph pipeline["변환 파이프라인"]
        ...
        output["output/<br/>parquet · reader · cache<br/>chart_rpc · stats_rpc<br/>raw_rpc_duckdb · xlsx<br/>parquet_async"]
    end
...
trace --grpc-server
  │
  └─ tonic 서버 (port 50051/50053) — 11개 RPC
       │
       ...
       ├─ ReadParquet        → output/raw_rpc_duckdb.rs::query_raw_page (DuckDB) ↹ dispatch::query_raw_page (inmem fallback) — offset / keyset cursor
       ...
       ├─ GetRawLogLines     → BufReader 단일 패스 → ± context (멀티 GB OOM 안전)
       └─ ExportXlsx        → output/xlsx.rs::save_*_to_xlsx_split_streaming → ZIP → MinIO upload
```

...
let batch = read_async_projected(
    reader, &columns, time_range,
    "time", &["continuous"],
).await?;
...
-- time 컬럼은 이미 초 단위라 단순 차이가 곧 duration_seconds (나누기 1000 없음)
SELECT count(*), count(*) FILTER (WHERE action='send_req'),
       COALESCE(max(time) - min(time), 0.0)::DOUBLE,
       count(*) FILTER (WHERE action='send_req' AND continuous),
       count(*) FILTER (WHERE action='send_req' AND lba % 8 = 0 AND size > 0 AND size % 8 = 0),
...
옛 parquet (continuous 컬럼 없음) 는 SQL 빌더가 DESCRIBE 로 컬럼 존재 여부를 확인 후 `FALSE AS continuous` 폴백을 base view 에 박음 (`stats_rpc_duckdb.rs` 의 `has_continuous` 분기 + `duckdb_common::parquet_columns`). 컬럼 자체가 없으면 `TRY_CAST` 는 binder error 라 못 잡고, 빌더 단계에서 분기해야 함. 결과적으로 continuous_count = 0. 정확도가 필요하면 `--migrate` 로 parquet 재생성.
...
// fixture 12행이 time 1.0 ~ 12.0 (초 단위) → 11.0초가 정답
assert!((p.duration_seconds - 11.0).abs() < eps);
...
| 응답 크기 | 수 MB (정확히 `target_points` row, default 30K) | 수십 KB |
| 풀스캔 횟수 | 머티리얼라이즈 1 + overview 1 + decimate SQL 1 = 3 (+ Rust trim μs) | 머티리얼라이즈 1 + overview 1 + cmd_stats 1 = 3 |

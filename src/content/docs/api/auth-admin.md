...
DB 테이블 관리 API입니다. (Admin 전용)

### SetInfomation

| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/admin/db/sets` | Set 목록 |
| POST | `/api/admin/db/sets` | Set 추가 |
| PUT | `/api/admin/db/sets/{id}` | Set 수정 |
| DELETE | `/api/admin/db/sets/{id}` | Set 삭제 |

### SlotInfomation

| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/admin/db/slots` | 슬롯 목록 |
| PUT | `/api/admin/db/slots/{tentacleName}/{slotNumber}` | 슬롯 수정 |

### UFS Info (7개 테이블 통합)

`{table}` 파라미터: `oem`, `nandsize`, `density`, `celltype`, `nandtype`, `controller`, `ufsversion`

| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/admin/db/ufsinfo/{table}` | 항목 목록 |
| POST | `/api/admin/db/ufsinfo/{table}` | 항목 추가 |
| PUT | `/api/admin/db/ufsinfo/{table}/{id}` | 항목 수정 |
| DELETE | `/api/admin/db/ufsinfo/{table}/{id}` | 항목 삭제 |

**요청 (POST/PUT UFS Info):**

...
Pre-Command API는 명령어 템플릿 관리, 슬롯/TC별 등록, 즉시 실행 기능을 제공합니다.
...
템플릿을 삭제합니다. 해당 템플릿에 등록된 슬롯/TC 매핑도 CASCADE 삭제됩니다.
...
### GET `/api/pre-commands/tc`

특정 슬롯의 TC별 등록 현황을 조회합니다.

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| source | string | O | `compatibility` 또는 `performance` |
| slotIndex | int | O | 슬롯 인덱스 |
...
:::caution
TC Pre-Command 등록 시 해당 슬롯의 슬롯 Pre-Command는 프론트엔드에서 자동 해제됩니다. TC 단위와 슬롯 단위 제어가 혼합되지 않도록 하기 위함입니다.
:::

### POST `/api/pre-commands/tc/unassign`

특정 TC의 Pre-Command를 해제합니다.
...
TC 등록:
  GET    /tc?source=X&slotIndex=Y  TC별 등록 현황
  POST   /tc/assign                TC에 등록
  POST   /tc/unassign              TC 해제

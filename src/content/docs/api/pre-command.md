...
### GET `/api/pre-commands/slots?source={source}`

슬롯별 등록 현황.

```json
[{ "slotIndex": 0, "preCommandId": 3, "preCommandName": "tiotest 설치" }]
```

### POST `/api/pre-commands/slots/assign`

```json
{ "preCommandId": 3, "source": "compatibility", "slotNumbers": [0, 1, 3] }
```

### POST `/api/pre-commands/slots/unassign`

```json
{ "source": "compatibility", "slotNumbers": [0, 1, 3] }
```
...
### GET `/api/pre-commands/tc?source={source}&slotIndex={slotIndex}`
...
```json
{ "preCommandId": 3, "source": "compatibility", "slotIndex": 0, "tcPosition": 2 }
```
...
```json
{ "source": "compatibility", "slotIndex": 0, "tcPosition": 2 }
```
...
```json
{ "source": "compatibility", "slotIndex": 0, "tcPreCommandIds": "5,3,0" }
```
...
슬롯 등록:
  GET    /slots?source=X           슬롯별 등록 현황
  POST   /slots/assign             슬롯에 등록
  POST   /slots/unassign           슬롯 해제

TC 등록:
  GET    /tc?source=X&slotIndex=Y  TC Pre-Command 현황 (comma 문자열)

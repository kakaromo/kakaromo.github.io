...
| **time-bucket decimate** | **정확히 `target_points` row 보장** stratified 다운샘플링 — 시간축 등분 + 빈 bucket 시 row index fill. **보호 row 5종** 강제: min/max time + argmax(qd) + min/max dtoc. NaN/Inf time/dtoc 자동 스킵 |

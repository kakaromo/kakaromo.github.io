...
Android device 의 VFS → FS(f2fs/ext4) → block → UFS(SCSI) 경로를 한 줄 단위
로 추적해서 "누가 / 어떤 파일을 / 어떤 syscall 로 / 어떤 성격의 IO를 / 어디
까지 내려갔나"를 한 화면에서 본다. 커널 수정 없이 libbpf + CO-RE eBPF.
...
tracer 는 raw event 만 emit. 다음 분석은 host 도구 (예: 별도 Rust 분석기
